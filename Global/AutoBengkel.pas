...
uses
  Classes, Controls, SysUtils, ADODB, SQLServerConnections, Subroutines;
...
{ cut from here ---------------------------------------------- 
  ...
  public
    ...
    property CarId: integer  read FCarId write FCarId;
  ...
  _PersonsCar_Arr = class(TObject) {Strong-Container Persons}
  private
    FOwner: TObject;
  protected
    FPersonsCar_Arr: array of _PersonsCar;
    function Get(Index: integer): _PersonsCar; virtual;
  public
    property Owner: TObject read FOwner;
    constructor Create(AOwner: TObject = nil);
    destructor Destroy; override;
    procedure Clear; virtual;
    function Count: integer;
    property PersonsCar_Arr[Index: integer]: _PersonsCar read Get; default;

    function Add(APersonId: integer; ANoPol: string; AMotorId: integer;AMtrYear: integer; AMtrRangka: string; AMtrMesin: string): integer; virtual;
    function IndexOf(APersonId: integer): integer; virtual;
    procedure Delete(Index: integer); virtual;
  end;
  ...
  _PersonsCar = class(TObject) {Strong-Basic Persons}
  private
    FOwner: TObject;
  protected
    FPersonId: integer; //<sequence
  ...
  public
    ...
    procedure DeleteOnDB; virtual;
    property PersonId: integer  read FPersonId write FPersonId;
    property NoPol: string  read FNoPol write FNoPol;
    property Motor: _Motor  read FMotor;
    property MtrYear: integer  read FMtrYear write FMtrYear;
    property MtrRangka: string  read FMtrRangka write FMtrRangka;
    property MtrMesin: string  read FMtrMesin write FMtrMesin;
    property Disable: integer  read FDisable write FDisable;
  ...
  public
    ...
    property Kilometer:string read FKilometer write FKilometer;
    property NoPol:string read FNoPol write FNoPol;
    property EstimasiId:integer read FEstimasiId write FEstimasiId;
  ...
  _ShipmentMechanic = class(TObject)
  private
    FOwner: TObject;
  protected
    FShipId: integer; //ShipId: integer
    FPersonID: integer; //
  public
    property Owner: TObject read FOwner;
    constructor Create(AOwner: TObject = nil);
  ...
  _ShipmentMechanic_Arr = class(TObject) {Strong-Container ItemDetail}
  private
    FOwner: TObject;
  protected
    FShipmentMechanic_Arr: array of _ShipmentMechanic;
    function Get(Index: integer): _ShipmentMechanic; virtual;
  public
    property Owner: TObject read FOwner;
    constructor Create(AOwner: TObject = nil);
    destructor Destroy; override;
    procedure Clear; virtual;
    function Count: integer;
    property ShipmentMechanic_Arr[Index: integer]: _ShipmentMechanic read Get; default;

    function Add(AShipId: integer; APersonId: integer): integer; virtual;
    function IndexOf(AShipId: integer): integer; virtual;
    procedure Delete(Index: integer); virtual;
  end;
...
constructor _PersonsCar_Arr.Create(AOwner: TObject);
begin
  inherited Create;
  FOwner := AOwner;
  setLength(FPersonsCar_Arr,0);
end;

destructor _PersonsCar_Arr.Destroy;
begin
  Self.Clear;
  inherited;
end;

procedure _PersonsCar_Arr.Clear;
var i: integer;
begin
  for i:=0 to high(FPersonsCar_Arr) do FPersonsCar_Arr[i].Destroy;
  setLength(FPersonsCar_Arr,0);
end;

function _PersonsCar_Arr.Add(APersonId: integer; ANoPol: string; AMotorId: integer;  AMtrYear: integer; AMtrRangka: string; AMtrMesin: string): integer;
begin
  setLength(FPersonsCar_Arr, length(FPersonsCar_Arr)+1);
  Result := high(FPersonsCar_Arr);
  FPersonsCar_Arr[Result] := _PersonsCar.Create(Self);
  FPersonsCar_Arr[Result].PersonId := APersonId;
  FPersonsCar_Arr[Result].Nopol := ANoPol;
  FPersonsCar_Arr[Result].Motor.MotorId := AMotorId;
  FPersonsCar_Arr[Result].MtrYear := AMtrYear;
  FPersonsCar_Arr[Result].MtrRangka := AMtrRangka;
  FPersonsCar_Arr[Result].MtrMesin := AMtrMesin;
end;

function _PersonsCar_Arr.IndexOf(APersonId: integer): integer;
var i: integer; found: boolean;
begin
  i := 0; found := false;
  while (i<=high(FPersonsCar_Arr)) and (not found) do
    if (FPersonsCar_Arr[i].FPersonId=APersonId) then
      found := true
    else inc(i);
  if (found) then
    Result := i
...
procedure _PersonsCar_Arr.Delete(Index: integer);
var i: integer;
begin
  if (Index<0) or (Index>high(FPersonsCar_Arr)) then Exit;
  FPersonsCar_Arr[Index].Destroy;
  for i:=Index to high(FPersonsCar_Arr)-1 do
    FPersonsCar_Arr[Index] := FPersonsCar_Arr[Index-1];
  setLength(FPersonsCar_Arr,length(FPersonsCar_Arr)-1);
end;

function _PersonsCar_Arr.Count: integer;
begin
  Result := length(FPersonsCar_Arr);
end;

function _PersonsCar_Arr.Get(Index: integer): _PersonsCar;
begin
  if (Index<0) or (Index>high(FPersonsCar_Arr)) then Result := nil
  else Result := FPersonsCar_Arr[Index];
end;
...
constructor _PersonsCar.Create(AOwner: TObject);
begin
  inherited Create;
  FOwner := AOwner;
  FMotor := _Motor.Create(Self);
...
destructor _PersonsCar.Destroy;
begin
  FMotor.Destroy;
  inherited;
end;

procedure _PersonsCar.Reset;
begin
  FPersonId := 0;
  FNoPol := '';
  FMotor.Reset;
...
procedure _PersonsCar.InsertOnDB;
begin
  try myConnection.ExecSQL(
    'INSERT INTO Persons_car (nopol,,Motor_Id,Mtr_Year,Mtr_Rangka,'+
    'Mtr_Mesin)'+
    ' VALUES ('+
...
procedure _PersonsCar.UpdateOnDB;
begin
  try myConnection.ExecSQL(
    'UPDATE Persons_Car SET'+
...
procedure _PersonsCar.DeleteOnDB;
begin
  try myConnection.ExecSQL(
    'DELETE FROM Persons_Car'+
    ' WHERE Person_Id='+FormatSQLNumber(FPersonId)
  ); except raise; end;
end;

function _PersonsCar.SelectInDB(APersonId: integer): boolean;
var buffer: _RecordSet;
begin
  buffer := myConnection.OpenSQL(
    'SELECT P.Person_Id, P.Person_Code, P.Person_Type, P.Person_Name, P.Address, P.City, P.Postal_Code, P.Birth_Date, P.Id_Num, P.Phone1, P.Phone2, P.Phone3, P.Notes, P.User_Insert, P.User_Edit, P.Date_Insert, P.Date_Edit'+
...
class function _PersonsCar.ExistInDB(APersonId: integer): integer;
var buffer: _RecordSet;
begin
  buffer := myConnection.OpenSQL(
    'SELECT COUNT(*) FROM Persons'+
...
{ _ShipmentMechanic_Arr }

function _ShipmentMechanic_Arr.Add(AShipId, APersonId: integer): integer;
begin
  setLength(FShipmentMechanic_Arr, length(FShipmentMechanic_Arr)+1);
  Result := high(FShipmentMechanic_Arr);
  FShipmentMechanic_Arr[Result] := _ShipmentMechanic.Create(Self);
  FShipmentMechanic_Arr[Result].FShipId := AShipId;
  FShipmentMechanic_Arr[Result].FPersonID := APersonId;
end;

procedure _ShipmentMechanic_Arr.Clear;
var i: integer;
begin
  for i:=0 to high(FShipmentMechanic_Arr) do FShipmentMechanic_Arr[i].Destroy;
  setLength(FShipmentMechanic_Arr,0);

end;

function _ShipmentMechanic_Arr.Count: integer;
begin
  Result := length(FShipmentMechanic_Arr);
end;

constructor _ShipmentMechanic_Arr.Create(AOwner: TObject);
begin
  inherited Create;
  FOwner := AOwner;
  setLength(FShipmentMechanic_Arr,0);
...
procedure _ShipmentMechanic_Arr.Delete(Index: integer);
var i: integer;
begin
  if (Index<0) or (Index>high(FShipmentMechanic_Arr)) then Exit;
  FShipmentMechanic_Arr[Index].Destroy;
  for i:=Index to high(FShipmentMechanic_Arr)-1 do
    FShipmentMechanic_Arr[Index] := FShipmentMechanic_Arr[Index-1];
  setLength(FShipmentMechanic_Arr,length(FShipmentMechanic_Arr)-1);

end;

destructor _ShipmentMechanic_Arr.Destroy;
begin
    Self.Clear;
  inherited;
end;

function _ShipmentMechanic_Arr.Get(Index: integer): _ShipmentMechanic;
begin
 if (Index<0) or (Index>high(FShipmentMechanic_Arr)) then Result := nil
  else Result := FShipmentMechanic_Arr[Index];
end;

function _ShipmentMechanic_Arr.IndexOf(AShipId: integer): integer;
var i: integer; found: boolean;
begin
  i := 0; found := false;
  while (i<=high(FShipmentMechanic_Arr)) and (not found) do
    if (FShipmentMechanic_Arr[i].FShipId=AShipId) then
      found := true
    else inc(i);
  if (found) then
    Result := i
...
constructor _ShipmentMechanic.Create(AOwner: TObject);
begin
  inherited Create;
  FOwner := AOwner;
end;

procedure _ShipmentMechanic.DeleteOnDB;
begin

end;

destructor _ShipmentMechanic.Destroy;
begin

  inherited;
end;

procedure _ShipmentMechanic.InsertOnDB;
begin
   try myConnection.ExecSQL(
    'INSERT INTO Shipment_Mechanic(ship_id,person_id)'+
    ' VALUES ('+
      FormatSQLNumber(FShipId)+','+
...
procedure _ShipmentMechanic.Reset;
begin
  FShipId := 0;
  FPersonID := 0;
end;

procedure _ShipmentMechanic.UpdateOnDB;
begin
