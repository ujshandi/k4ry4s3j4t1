declare cr_item_detail cursor local for
	...
	where s.ship_type in ('K','S','V') and d.price > 0 and d.item_id is not null 
	order by ship_date;
...
while @@fetch_status = 0 begin
  ...
  insert into item_price(item_id,price,user_insert, date_insert)
  values (@item_id, @price,@userInsert, @dateEdit)
  fetch next from cr_item_detail into @item_id,@price_purchase, @price, @userInsert, @dateEdit
end
