...
procedure TqrpMutasi.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  vTambahanRow := 0;
  gNoUrut := 0;
...
procedure TqrpMutasi.QRDBText14Print(sender: TObject; var Value: String);
begin
  if Value <> '' then
    gNoUrut := gNoUrut + 1;
...
function TqrpMutasi.NeoLoadData(AOfferSeq:integer): Boolean;
var
  Sql: String;
begin
...
procedure TqrpMutasi.QRDBText9Print(sender: TObject; var Value: String);
begin
  Value := Value + ' Hari';
end;


procedure TqrpMutasi.SetFooterBand;
begin
...
procedure TqrpMutasi.SummaryBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  SetFooterBand;
end;

procedure TqrpMutasi.Executes(AServiceSeq:integer;APurpose:string);
...
begin
  ...
  qrlAddress.Caption := vCompany.Address +' '+vCompany.City+' Telp. '+ vCompany.Telp1+
                        ' Fax. '+vCompany.Fax;
  qrlTitle.Caption   := 'Mutasi Barang'+IfThen(APurpose= SHIP_TYPE_MUTASI_MASUK,' (Masuk)',' (Keluar)');
...
procedure TqrpMutasi.SetColumHeader(AType: String);
begin
...
procedure TqrpMutasi.SetDetail(ANomor: integer; ANama: string;
...
procedure TqrpMutasi.SetEmpty(AType:string);
begin
...
procedure TqrpMutasi.SetFooter(AType:string;ATotal,ADisc,ANett:Double);
begin
  qrlNomor.Font.Style    := [];
  qrlQty.Font.Style      := [];
  qrlPrice.Font.Style    := [];
...
procedure TqrpMutasi.SetColumHeaderAll(AType: string);
begin
  qrlNomor.Font.Style    := [fsBold];
  qrlQty.Font.Style      := [fsBold];
  qrlPrice.Font.Style    := [fsBold];
...
procedure TqrpMutasi.SetDetailAll(ANomor:integer;ANama,ACode:string;AQty,APrice,AAMount,ADisc:real;AType:string);
begin
  qrlNomor.Font.Style  := [];
  qrlQty.Font.Style    := [];
  qrlPrice.Font.Style  := [];
...
procedure TqrpMutasi.QuickRepNeedData(Sender: TObject;
...
procedure TqrpMutasi.QRLabel3Print(sender: TObject; var Value: String);
begin
