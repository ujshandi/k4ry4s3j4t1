...
type
  TqrpNotaReturBeli = class(TQuickRep)
    TitleBand1: TQRBand;
    ...
    QRDBText4: TQRDBText;
    QRLabel2: TQRLabel;
    adoMastership_id_used: TLargeintField;
    qrlNoterima: TQRLabel;
    ...
    procedure qrlNoterimaPrint(sender: TObject; var Value: String);
  private
...
var
  qrpNotaReturBeli: TqrpNotaReturBeli;
...
procedure TqrpNotaReturBeli.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  vTambahanRow := 0;
  gNoUrut := 0;
...
procedure TqrpNotaReturBeli.QRDBText14Print(sender: TObject; var Value: String);
begin
  if Value <> '' then
    gNoUrut := gNoUrut + 1;
...
function TqrpNotaReturBeli.NeoLoadData(AOfferSeq:integer): Boolean;
var
  Sql: String;
begin
  ...
  Sql := 'SELECT TOP 100 PERCENT s.ship_id, s.ship_num, s.ship_type, s.user_insert, s.ship_date, s.person_id, P.person_name, P.address, P.city, P.phone1, s.total, '+
         ' s.notes, b.item_id, i.item_code, i.item_name, d.quantity, d.price AS price_tampil, d.amount, ''B'' AS tipe, d.item_detail_id AS detail_seq, '+
          ...
          ', s.tgl_kirim, s.tgl_sampai, s.delivery_num,s.ship_id_used '+
...
procedure TqrpNotaReturBeli.QRDBText9Print(sender: TObject; var Value: String);
begin
  Value := Value + ' Hari';
end;


procedure TqrpNotaReturBeli.SetFooterBand;
begin
...
procedure TqrpNotaReturBeli.SummaryBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  SetFooterBand;
end;

procedure TqrpNotaReturBeli.Executes(AServiceSeq:integer);
...
procedure TqrpNotaReturBeli.SetColumHeader(AType: String);
begin
...
procedure TqrpNotaReturBeli.SetDetail(ANomor: integer; ANama: string;
...
procedure TqrpNotaReturBeli.SetEmpty(AType:string);
begin
...
procedure TqrpNotaReturBeli.SetFooter(AType:string;ATotal,ADisc,ANett:Double);
begin
  qrlNomor.Font.Style    := [];
  qrlQty.Font.Style      := [];
  qrlPrice.Font.Style    := [];
...
procedure TqrpNotaReturBeli.SetColumHeaderAll(AType: string);
begin
  qrlNomor.Font.Style    := [fsBold];
  qrlQty.Font.Style      := [fsBold];
  qrlPrice.Font.Style    := [fsBold];
...
procedure TqrpNotaReturBeli.SetDetailAll(ANomor:integer;ANama,ACode:string;AQty,APrice,AAMount,ADisc:real;AType:string);
begin
  qrlNomor.Font.Style  := [];
  qrlQty.Font.Style    := [];
  qrlPrice.Font.Style  := [];
...
procedure TqrpNotaReturBeli.QuickRepNeedData(Sender: TObject;
...
procedure TqrpNotaReturBeli.qrlNoterimaPrint(sender: TObject;
  var Value: String);
begin
   Value := TShipment.getNumFromID(adoMastership_id_used.AsInteger);
end;
