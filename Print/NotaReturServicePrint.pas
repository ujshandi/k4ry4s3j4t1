...
type
  TqrpNotaReturService = class(TQuickRep)
    TitleBand1: TQRBand;
...
var
  qrpNotaReturService: TqrpNotaReturService;
...
procedure TqrpNotaReturService.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  gNoUrut := 0;
...
procedure TqrpNotaReturService.QRDBText14Print(sender: TObject; var Value: String);
begin
  if Value <> '' then
    gNoUrut := gNoUrut + 1;
...
function TqrpNotaReturService.NeoLoadData(AOfferSeq:integer): Boolean;
var
  Sql, vFilter: String;
begin
  vFilter := '';
  ...
  qrlNoService.Caption := TShipment.getShipNumRefFromId(AOfferSeq);
  qrlMekanik.Caption    := TShipment.getMekanikFromIdRetur(AOfferSeq);
  ...
  Sql :=  '   SELECT S.Ship_id, S.Ship_Num, S.Ship_Type, S.User_insert, S.Ship_Date,'+
          ...
          ' ,d.service_parent, s.ship_service_type, s.down_payment,s.is_post '+
  ...
  ReportTitle := 'Nota Retur Service';
  qrlNetto.Caption := IfThen(adoMasteris_post.Value=TRUE_VALUE,'Netto','Sisa');
end;

procedure TqrpNotaReturService.QRDBText9Print(sender: TObject; var Value: String);
begin
  Value := Value + ' Hari';
end;


procedure TqrpNotaReturService.SetFooterBand;
begin
  qrlSUmDp.Caption   := FloatToStrFmt2(adoMasterdown_payment.Value);
  qrSumTotal.Caption := FloatToStrFmt2(vGrantTotal);//adoMastersubtotal.Value); //
  qrlSumDisc.Caption := FloatToStrFmt2(adoMasterdisc_faktur.Value);// vGrandDisc);
  qrlSumNet.Caption := FloatToStrFmt2(vGrantNett - adoMasterdown_payment.Value - adoMasterdisc_faktur.Value);//adoMastertotal.Value);
end;

procedure TqrpNotaReturService.SummaryBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  SetFooterBand;
end;

procedure TqrpNotaReturService.Executes(AServiceSeq:integer);
...
procedure TqrpNotaReturService.QuickRepNeedData(Sender: TObject;
...
//          if (vRealRow = (ROW_BARU * (Printer.PageNumber-1))) then
        ...
        Inc(gNoUrut);
         if (adoMastership_service_type.Value = SHIP_SERVICE_TYPE_KSG) and
           ...
           vTotal := vTotal + adoMasterAmount.Value{(adoMasteraquantity.Value *
                adoMasterprice_tampil.Value)} + vTotalTransisi;
           vDisc  := 0;//vDisc + adoMasterdiscount.Value + vDiscTransisi;
...
procedure TqrpNotaReturService.SetEmpty(AType:string);
begin
...
procedure TqrpNotaReturService.SetFooter(AType:string;ATotal,ADisc,ANett:Double;AShowShape:Boolean);
begin
...
procedure TqrpNotaReturService.SetColumHeaderAll(AType: string);
begin
  qrlNomor.Font.Style    := [fsBold];
  qrlQty.Font.Style      := [fsBold];
  qrlPrice.Font.Style    := [fsBold];
...
procedure TqrpNotaReturService.SetDetailAll(ANomor:integer;ANama,ACode:string;AQty,APrice,AAMount,ADisc:real;AType:string;
              aItemId,aServiceParent:integer;aShipType:string);
begin
  qrlNomor.Font.Style  := [];
  qrlQty.Font.Style    := [];
