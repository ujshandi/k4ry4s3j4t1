...
type
  TqrpEstimasi = class(TQuickRep)
    ...
    qrlRow: TQRLabel;
    qrlRealRow: TQRLabel;
    adoMasterEstimasi_id: TLargeintField;
...
var
  qrpEstimasi: TqrpEstimasi;

implementation

uses SQLServerConnections, un_ryu, UConst, NotaServiceObjectPrint;
...
procedure TqrpEstimasi.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  isFooter := False;
  vTambahanRow := 0;
...
procedure TqrpEstimasi.QRDBText14Print(sender: TObject; var Value: String);
begin
 // if Value <> '' then
   // gNoUrut := gNoUrut + 1;
...
function TqrpEstimasi.NeoLoadData(AOfferSeq:integer): Boolean;
var
  Sql, vFilter: String;
begin
  vFilter := '';
  if AOfferSeq <> 0 then
    vFilter := vFilter + ' AND S.Estimasi_id ='+FormatSQLNumber(AOfferSeq);



  Sql :=  '   SELECT S.Estimasi_id, S.Ship_Num, S.Ship_Type, S.User_insert, S.Ship_Date,'+
          ...
          ' d.Amount, ''A'' as tipe,Estimasi_service_detail_id as detail_seq,dbo.get_mekanik(s.mekanik_id) as mekanik,p.Person_code, '+
          ' m.motor_name, p.mtr_year, p.mtr_rangka, p.mtr_mesin, d.discount, s.discount as disc_faktur, s.subtotal '+
          ' ,d.service_parent, s.ship_service_type, s.down_payment,s.is_post,s.expr_Date, d.is_ksg,S.nopol,S.Kilometer, ''pcs'' as satuan '+
           ' FROM dbo.Estimasi s INNER JOIN '+
           '  dbo.Persons P ON s.person_id = P.person_id INNER JOIN '+
           '  dbo.Estimasi_service_detail d ON s.Estimasi_id = d.Estimasi_id INNER JOIN '+
           ...
           ' WHERE     (s.Estimasi_id = '+FormatSQLNumber(AOfferSeq)+')' +
          ' UNION '+
          'SELECT S.Estimasi_id, S.Ship_Num, S.Ship_Type, S.User_insert, S.Ship_Date, '+
          ' S.Person_id, P.Person_name, P.address,p.city, P.Phone1,S.total,   S.Notes, '+
          ' d.item_id, i.item_code, i.item_name, d.quantity, d.price as price_tampil,'+
          ' d.Amount, ''B'' as tipe, estimasi_detail_id as detail_seq,dbo.get_mekanik(s.mekanik_id) as mekanik,p.Person_code,'+
          ' m.motor_name, p.mtr_year, p.mtr_rangka, p.mtr_mesin, d.discount, s.discount as disc_faktur, s.subtotal '+
          ' ,d.service_parent, s.ship_service_type, s.down_payment,s.is_post,s.expr_Date, d.is_ksg,S.nopol,S.Kilometer, i.used_unit as satuan '+
           ' FROM dbo.Estimasi s INNER JOIN '+
           '  dbo.Persons P ON s.person_id = P.person_id INNER JOIN '+
           '  dbo.Estimasi_Detail d ON s.Estimasi_id = d.Estimasi_id INNER JOIN '+
...
{           ' UNION '+
          ...
          ' ,d.service_parent, s.ship_service_type, s.down_payment,s.is_post,s.expr_Date, d.is_ksg,S.nopol,S.Kilometer, ''pcs'' as satuan '+
           ' FROM dbo.Estimasi s INNER JOIN '+
           '  dbo.Persons P ON s.person_id = P.person_id INNER JOIN '+
           '  dbo.dummy_item_detail d ON s.Estimasi_id = d.Estimasi_id INNER JOIN '+
           '  dbo.Services v ON v.Service_id = d.service_id LEFT OUTER JOIN '+
           '  dbo.persons_car PC  ON PC.person_id = s.person_id and PC.nopol = s.nopol INNER JOIN '+
           '  dbo.Motor m ON PC.motor_id = m.motor_id '+
           ' WHERE d.item_id IS NULL AND    (s.Estimasi_id = '+FormatSQLNumber(AOfferSeq)+')' +
          ' UNION '+
          'SELECT S.Estimasi_id, S.Ship_Num, S.Ship_Type, S.User_insert, S.Ship_Date, '+
          ' S.Person_id, P.Person_name, P.address,p.city, P.Phone1,S.total,   S.Notes, '+
          ' d.item_id, i.item_code, i.item_name, d.quantity, d.price as price_tampil,'+
       ...
       //   '  dbo.Item_Balance b ON d.item_balance_id = b.item_balance_id INNER JOIN '+
           '  dbo.Items i ON d.item_id = i.item_id LEFT OUTER JOIN '+
           '  dbo.Motor m ON PC.motor_id = m.motor_id '+
           ' WHERE  d.service_id IS NULL AND   (s.Estimasi_id = '+FormatSQLNumber(AOfferSeq)+')' ;
                            }

  Sql := Sql + ' Order By s.Estimasi_id,ship_date,tipe desc,item_code, detail_seq ';
  ...
  qrMemoMekanik.Lines.Text :=   TShipmentMechanic_Arr.getMechanic(adoMasterEstimasi_id.Value);
...
procedure TqrpEstimasi.QRDBText9Print(sender: TObject; var Value: String);
begin
  Value := Value + ' Hari';
end;


procedure TqrpEstimasi.SetFooterBand;
begin
...
procedure TqrpEstimasi.SummaryBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  SetFooterBand;
end;

procedure TqrpEstimasi.Executes(AServiceSeq:integer;isEstimasi:boolean=false);
...
procedure TqrpEstimasi.SetFooter(AType:string;ATotal,ADisc,ANett:Double;AShowShape:Boolean);
begin
...
procedure TqrpEstimasi.SetColumHeaderAll(AType: string);
begin
  qrlNomor.Font.Style    := [fsBold];
  qrlQty.Font.Style      := [fsBold];
  qrlPrice.Font.Style    := [fsBold];
  ...
  if AType = 'B' then begin// bahan
    ...
    qrlNama.Lines.Text := 'Jenis Service';
...
procedure TqrpEstimasi.SetDetailAll(ANomor:integer;ANama,ACode:string;AQty,APrice,AAMount,ADisc:real;AType:string;
              aItemId,aServiceParent:integer;aShipType:string;isKsg:boolean;satuan:string);
begin
  qrlNomor.Font.Style  := [];
  qrlQty.Font.Style    := [];
...
procedure TqrpEstimasi.QuickRepNeedData(Sender: TObject;
