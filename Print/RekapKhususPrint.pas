...
type
  TqrpRekapKhusus = class(TQuickRep)
    TitleBand1: TQRBand;
...
var
  qrpRekapKhusus: TqrpRekapKhusus;
...
procedure TqrpRekapKhusus.Executes(AGrid: TAdvStringGrid;APurpose:string);
begin
  ...
  qrlTitle.Caption := IfThen(Apurpose=SHIP_TYPE_SALES,'Penjualan Khusus [Sales]','Penjualan Khusus [Service]');
  ReportTitle := qrlTitle.Caption;
  PreviewModal;
end;

procedure TqrpRekapKhusus.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  i:= 1;
end;
...
procedure TqrpRekapKhusus.QuickRepNeedData(Sender: TObject;
  var MoreData: Boolean);
begin
  MoreData := i < MasterGrid.RowCount-1;
  if MoreData then begin

    qrl0.Caption := MasterGrid.Cells[colKNo,i];
    qrl1.Caption := MasterGrid.Cells[colKDate,i];
    qrl2.Caption := MasterGrid.Cells[colKCust,i];
    qrl3.Caption := MasterGrid.Cells[colKPartCode,i];
    qrl4.Caption := MasterGrid.Cells[colKPartName,i];
    qrl5.Caption := MasterGrid.Cells[colKPriceSales,i];
    qrl6.Caption := MasterGrid.Cells[colKPriceHpp,i];
    qrl7.Caption := MasterGrid.Cells[colKQty,i];
    qrl8.Caption := MasterGrid.Cells[colKDisc,i];
    qrl9.Caption := MasterGrid.Cells[colKTotal,i];
   // qrl0.Caption := MasterGrid.Cells[colKNo,i];
//      setDetailItem(i);
      Inc(i)
  END;
...
procedure TqrpRekapKhusus.SummaryBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  qrlSumQty.Caption := MasterGrid.Cells[colKQty,MasterGrid.rowCount-1];
  qrlSUmTotal.Caption := MasterGrid.Cells[colKTotal,MasterGrid.rowCount-1];
end;
