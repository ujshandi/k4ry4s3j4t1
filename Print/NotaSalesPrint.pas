...
procedure TqrpNotaSales.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  vTambahanRow := 0;
  gNoUrut := 0;
...
procedure TqrpNotaSales.QRDBText14Print(sender: TObject; var Value: String);
begin
  if Value <> '' then
    gNoUrut := gNoUrut + 1;
...
function TqrpNotaSales.NeoLoadData(AOfferSeq:integer): Boolean;
var
  Sql: String;
begin
//  vFilter := '';
         ...
         ' s.notes, b.item_id, i.item_code, i.item_name, d.quantity, d.price AS price_tampil, d.amount, ''B'' AS tipe, d.item_detail_id AS detail_seq, '+
          ...
          ' FROM dbo.Shipment s INNER JOIN '+
           ...
           '  dbo.Items i ON b.item_id = i.item_id LEFT OUTER JOIN '+
             '  dbo.persons_car PC  ON PC.person_id = s.person_id and PC.nopol = s.nopol LEFT OUTER JOIN '+
           ...
           ' INNER JOIN   dbo.Items i ON d.item_id = i.item_id LEFT OUTER JOIN '+
              '  dbo.persons_car PC  ON PC.person_id = s.person_id and PC.nopol = s.nopol LEFT OUTER JOIN '+
           '  dbo.Motor m ON PC.motor_id = m.motor_id  '+
...
procedure TqrpNotaSales.QRDBText9Print(sender: TObject; var Value: String);
begin
  Value := Value + ' Hari';
end;


procedure TqrpNotaSales.SetFooterBand;
begin
...
procedure TqrpNotaSales.SummaryBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  SetFooterBand;
end;

procedure TqrpNotaSales.Executes(AServiceSeq:integer);
...
procedure TqrpNotaSales.SetColumHeader(AType: String);
begin
...
procedure TqrpNotaSales.SetDetail(ANomor: integer; ANama: string;
...
{procedure TqrpNotaSales.QuickRepNeedData(Sender: TObject;
  ...
  if MoreData then begin
    ...
    if vTipe <> adoMastertipe.Value then begin
      ...
      if (vPosisiRecord = adoMaster.RecordCount) OR (vRow = TOTAL_ROW) or
        ...
        else begin  //jika counter sudah >=4                -1
          vAwalPage := (TOTAL_ROW * (Self.Printer.PageNumber-1) + (1 * (Self.Printer.PageNumber)));//-
...
//          if (vRealRow = (ROW_BARU * (Printer.PageNumber-1))) then
            ...
            else begin                              //vNeet di masukin ke vQTy
               SetDetailAll(-1,'Total Sebelumnya ','',vNettTransisi,0,vTotalTransisi,vDiscTransisi,adoMastertipe.Value);
...
procedure TqrpNotaSales.SetEmpty(AType:string);
begin
...
procedure TqrpNotaSales.SetFooter(AType:string;ATotal,ADisc,ANett:Double);
begin
  qrlNomor.Font.Style    := [];
  qrlQty.Font.Style      := [];
  qrlPrice.Font.Style    := [];
...
procedure TqrpNotaSales.SetColumHeaderAll(AType: string);
begin
  qrlNomor.Font.Style    := [fsBold];
  qrlQty.Font.Style      := [fsBold];
  qrlPrice.Font.Style    := [fsBold];
...
procedure TqrpNotaSales.SetDetailAll(ANomor:integer;ANama,ACode:string;AQty,APrice,AAMount,ADisc:real;AType:string);
begin
  qrlNomor.Font.Style  := [];
  qrlQty.Font.Style    := [];
  qrlPrice.Font.Style  := [];
