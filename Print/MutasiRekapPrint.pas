...
type
  TqrpMutasiRekap = class(TQuickRep)
    TitleBand1: TQRBand;
    ...
    adoqMasterware: TStringField;
    adoqMasterware_out: TStringField;
    qrmCompany: TQRMemo;
    qrlPeriod: TQRLabel;
  ...
  public
     ...
     procedure ExecutQuery(APurpose:string;aGudangOut,aGudangIn:integer);

  end;

var
  qrpMutasiRekap: TqrpMutasiRekap;
...
procedure TqrpMutasiRekap.Executes(AGrid: TAdvStringGrid);
begin
...
procedure TqrpMutasiRekap.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
...
procedure TqrpMutasiRekap.setDetailHeader;
begin
...
procedure TqrpMutasiRekap.setDetailItem(ARow: integer);
begin
  shapeHeader.Enabled := True;
...
procedure TqrpMutasiRekap.setDetailTrans(AIndex: integer);
begin
...
procedure TqrpMutasiRekap.QuickRepNeedData(Sender: TObject;
...
procedure TqrpMutasiRekap.ExecutQuery(APurpose:string;aGudangOut,aGudangIn:integer);
var vSQL,vFilter : string;
begin
  qrmCompany.Lines.Text := TCompanyProfile.getProfile;
  qrlPeriod.Caption := CaptionPeriode(GlobalPeriode.OpPeriodeAwal1,GlobalPeriode.PeriodeAwal1,GlobalPeriode.PeriodeAkhir1);
  vSQL := 'Select ship_date, ship_num, s.person_id, p.person_name, ship_id '+
           ',  s.notes, dbo.get_warehouse(wh_id) as ware, dbo.get_warehouse(wh_id_out) as ware_out '+
  ...
  if GlobalPeriode.PeriodeAkhir1 <> 0 then
   ...
   if aGudangIn <> 0 then
    vFilter := vFilter +' AND s.wh_id ='+FormatSQLNumber(aGudangIn);
  if aGudangOut <> 0 then
    vFilter := vFilter +' AND s.wh_id_out ='+FormatSQLNumber(aGudangOut);
   if vFilter <> '' then
    vSQL := vSQL + vFilter;
  ...
  qrdbWare.DataField := IfThen(APurpose=SHIP_TYPE_MUTASI_MASUK,'ware','ware_out');
  qrlTitle.Caption := 'Rekap Mutasi '+IfThen(APurpose=SHIP_TYPE_MUTASI_MASUK,'Masuk','Keluar');
  lblGdg.Caption := IfThen(APurpose=SHIP_TYPE_MUTASI_MASUK,'Dari','Ke')+' Gudang';
  ReportTitle := qrlTitle.Caption;
  PreviewModal;
end;

procedure TqrpMutasiRekap.DetailBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
   qrNoSUbDetail.Reset;
end;
