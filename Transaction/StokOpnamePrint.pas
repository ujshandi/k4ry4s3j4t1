...
type
  TqrpOpname = class(TQuickRep)
    ...
    QRLabel2: TQRLabel;
    lblGudang: TQRLabel;
    adoMasterwh_id: TIntegerField;
    adoMasterwh_id_out: TIntegerField;
    QRLabel3: TQRLabel;
...
//    procedure Executes(AOfferArr: TOffer_Arr; ADetail: Boolean);
    procedure Executes(AServiceSeq:integer;APurpose:string);
  end;

var
  qrpOpname: TqrpOpname;

implementation

uses SQLServerConnections, un_ryu, UConst;

const TOTAL_ROW = 48;//40;
      ROW_BARU  = 56;//48;
      BATAS_EMPTY = 1;
{$R *.DFM}
...
procedure TqrpOpname.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  vTambahanRow := 0;
  gNoUrut := 0;
...
procedure TqrpOpname.QRDBText14Print(sender: TObject; var Value: String);
begin
  if Value <> '' then
    gNoUrut := gNoUrut + 1;
...
function TqrpOpname.NeoLoadData(AOfferSeq:integer): Boolean;
var
  Sql: String;
begin
   Sql := 'SELECT TOP 100 PERCENT s.ship_id, s.ship_num, s.ship_type, s.user_insert, s.ship_date, s.person_id, P.person_name, P.address, P.city, P.phone1, s.total, '+
         ' s.notes, b.item_id, i.item_code, i.item_name, d.quantity, d.price AS price_tampil, d.amount, ''B'' AS tipe, d.item_detail_id AS detail_seq, '+
          ...
          ', s.tgl_kirim, s.tgl_sampai, s.delivery_num ,s.wh_id, s.wh_id_out     '+
          ' FROM dbo.Shipment s LEFT JOIN '+
...
procedure TqrpOpname.QRDBText9Print(sender: TObject; var Value: String);
begin
  Value := Value + ' Hari';
end;


procedure TqrpOpname.SetFooterBand;
begin
 // qrlSUmDp.Caption   := FloatToStrFmt2(adoMasterdown_payment.Value);
  qrSumTotal.Caption := FloatToStrFmt2(vGrantTotal);// adoMastersubtotal.Value);// vGrantTotal);//adoMastertotal.Value);
...
procedure TqrpOpname.SummaryBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  SetFooterBand;
end;

procedure TqrpOpname.Executes(AServiceSeq:integer;APurpose:string);
...
begin
  ...
  qrlAddress.Caption := vCompany.Address +' '+vCompany.City+' Telp. '+ vCompany.Telp1+
                        ' Fax. '+vCompany.Fax;
  qrlTitle.Caption   := 'Stok Opname';//'Mutasi Barang'+IfThen(APurpose= SHIP_TYPE_MUTASI_MASUK,' (Masuk)',' (Keluar)');
  lblGudang.Caption := IfThen(APurpose=SHIP_TYPE_MUTASI_MASUK,'Gudang Asal','Gudang Tujuan');
...
procedure TqrpOpname.SetColumHeader(AType: String);
begin
...
procedure TqrpOpname.SetDetail(ANomor: integer; ANama: string;
...
procedure TqrpOpname.SetEmpty(AType:string);
begin
...
procedure TqrpOpname.SetFooter(AType:string;ATotal,ADisc,ANett:Double);
begin
  qrlNomor.Font.Style    := [];
  qrlQty.Font.Style      := [];
  qrlPrice.Font.Style    := [];
...
procedure TqrpOpname.SetColumHeaderAll(AType: string);
begin
  qrlNomor.Font.Style    := [fsBold];
  qrlQty.Font.Style      := [fsBold];
  qrlPrice.Font.Style    := [fsBold];
...
procedure TqrpOpname.SetDetailAll(ANomor:integer;ANama,ACode:string;AQty,APrice,AAMount,ADisc:real;AType:string);
begin
  qrlNomor.Font.Style  := [];
  qrlQty.Font.Style    := [];
  qrlPrice.Font.Style  := [];
...
procedure TqrpOpname.QuickRepNeedData(Sender: TObject;
...
procedure TqrpOpname.QRLabel3Print(sender: TObject; var Value: String);
begin
  if  MasterPurpose = SHIP_TYPE_MUTASI_MASUK then
     Value := TWarehouse.getName(adoMasterWh_Id.Value)
   else
     Value := TWarehouse.getName(adoMasterWh_Id_out.Value)
end;
