...
type
  TfrmServices = class(TForm)
    ...
    Label2: TLabel;
    txtKilometer: TAdvEdit;
    TabSheet1: TTabSheet;
    asgMekanik: TAdvStringGrid;
    Label3: TLabel;
    txtOrder: TAdvEdit;
    btnOrder: TButton;
    cmbNoPol: TComboBox;
    ...
    procedure chkPostKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure cmbMekanikKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    ...
    procedure asgRegPartKeyPress(Sender: TObject; var Key: Char);
...
uses Subroutines, MainMenu, un_ryu, UConst, UTransaction,
  ...
  PersonAdd, NotaServiceObjectPrint, LOVFlat, PersonCarAdd,
  NotaServiceObjectGabungPrint;
...
procedure TfrmServices.ArrangeColSize;
begin
...
function TfrmServices.Execute(ATerimaSeq: integer;AEditMode:boolean): boolean;
begin
...
begin
 // btnSave.Enabled   := not BrowseMode;
  ...
  cmbMekanik.Enabled := (not BrowseMode) or EditMode;
  ...
  Self.Caption := IfThen(BrowseMode,'View Service','New Service');
...
function TfrmServices.isSaved: boolean;
...
begin
   ...
   vShipment.ShipNum := IfThen(EditMode,Trim(sttNum.Caption),TShipment.getNewNumberService(SHIP_TYPE_SERVICE,MasterShipService,dtpTrans.Date));//sttNum.Caption;
   vShipment.ShipType:= SHIP_TYPE_SERVICE;
   ...
   vShipment.Kilometer := txtKilometer.Text;
   ...
   vShipment.EstimasiId := txtOrder.Tag;
...
function TfrmServices.isValid: boolean;
...
procedure TfrmServices.LoadData;
var vShipment : TShipment;
   // vDetail   : TItemDetail_Arr;
    i,vRow:integer;
...
begin
   ...
   cmbMekanik.ItemIndex   := ListMekanik.IndexOfName(IntToStr(vShipment.PersonsMekanik_Id.PersonId));
   ...
   txtKilometer.Text  := vShipment.Kilometer;
...
procedure TfrmServices.NewNumber;
begin
  if not BrowseMode then
    sttNum.Caption := ' '+TShipment.getNewNumberService(SHIP_TYPE_SERVICE,MasterShipService,dtpTrans.DateTime);
end;

procedure TfrmServices.asgRegPartCanEditCell(Sender: TObject; ARow,
  ACol: Integer; var CanEdit: Boolean);
begin
...
procedure TfrmServices.asgRegPartCellValidate(Sender: TObject; Col,
  Row: Integer; var Value: String; var Valid: Boolean);
...
begin
  case Col of
    ...
    colQty :begin
      ...
      if Valid then begin
        Valid := (StrFmtToFloatDef(Value,0) <= (TItems.getStokTotal(asgRegPart.Ints[colID,Row],True)+
...
procedure TfrmServices.asgRegPartGetAlignment(Sender: TObject; ARow,
  ACol: Integer; var HAlign: TAlignment; var VAlign: TVAlignment);
begin
  if (ARow = 0) then
    HAlign := taCenter
...
procedure TfrmServices.asgRegPartGetEditorType(Sender: TObject; ACol,
  ARow: Integer; var AEditor: TEditorType);
begin
  case ACol of
...
procedure TfrmServices.btnSaveClick(Sender: TObject);
begin
...
procedure TfrmServices.btnBatalClick(Sender: TObject);
begin
   ModalResult := mrCancel;
end;

procedure TfrmServices.dtpTransChange(Sender: TObject);
begin
  NewNumber;
  InitGrid;
end;

procedure TfrmServices.btnNewClick(Sender: TObject);
...
procedure TfrmServices.SetAmount(AGrid:TAdvStringGrid;ARow: integer;aCol:integer);
var i:integer;
    vState:boolean;
begin
...
procedure TfrmServices.SetFooter;
begin
...
procedure TfrmServices.txtDiscChange(Sender: TObject);
begin
  SetFooter;
end;

procedure TfrmServices.LoadCustomer(ANoPol: string);
var vCust : TPersons;
begin
...
procedure TfrmServices.btnCustClick(Sender: TObject);
var vTmp : integer;
begin
...
procedure TfrmServices.txtBayarChange(Sender: TObject);
begin
   txtkembali.FloatValue :=  txtBayar.FloatValue - txtTotal.FloatValue;
end;

procedure TfrmServices.PrintData;
var tmpId : integer;
begin
  tmpId := TShipment.getIdFromNum(Trim(sttNum.Caption));
 // if TShipment.isPosted(tmpId) then
   if rgbPrint.ItemIndex = 0 then begin
    Application.CreateForm(TqrpNotaServiceObject,qrpNotaServiceObject);
    qrpNotaServiceObject.Executes(tmpId);
    qrpNotaServiceObject.Free;
   end
   else begin
     Application.CreateForm(TqrpNotaServiceObjectGabung,qrpNotaServiceObjectGabung); 
    qrpNotaServiceObjectGabung.Executes(tmpId);
    qrpNotaServiceObjectGabung.Free;
   end;
 // else
...
procedure TfrmServices.asgCompanyPartCanEditCell(Sender: TObject; ARow,
  ACol: Integer; var CanEdit: Boolean);
begin
...
procedure TfrmServices.asgCompanyPartCellValidate(Sender: TObject; Col,
  Row: Integer; var Value: String; var Valid: Boolean);
   var vMsg:string;vId:integer;
begin
  case Col of
   {$IFDEF BARCODE}
    ...
    colQty :begin
      ...
      vMsg  := 'Data masukan salah!';
      if Valid then begin
        Valid := (StrFmtToFloatDef(Value,0) <= (TItems.getStokTotal(asgCompanyPart.Ints[colID,Row],True)+
...
procedure TfrmServices.asgCompanyPartGetEditorType(Sender: TObject; ACol,
  ARow: Integer; var AEditor: TEditorType);
begin
    case ACol of
...
procedure TfrmServices.pgcDetailChange(Sender: TObject);
begin
...
procedure TfrmServices.DeleteRow1Click(Sender: TObject);
...
procedure TfrmServices.asgRegPartRightClickCell(Sender: TObject; ARow,
  ACol: Integer);
begin
 // if ARow = 0 then exit;
...
procedure TfrmServices.asgCompanyPartRightClickCell(Sender: TObject; ARow,
  ACol: Integer);
begin
//  if ARow = 0 then exit;
  //popReguler.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
...
procedure TfrmServices.asgKSGPartCanEditCell(Sender: TObject; ARow,
  ACol: Integer; var CanEdit: Boolean);
begin
...
procedure TfrmServices.asgKSGPartCellValidate(Sender: TObject; Col,
  Row: Integer; var Value: String; var Valid: Boolean);
var vMsg:string; vId:integer;
begin
  case Col of
    ...
    colQty :begin
      ...
      vMsg  := 'Data masukan salah!';
      if Valid then begin
        Valid := (StrFmtToFloatDef(Value,0) <= (TItems.getStokTotal(asgKSGPart.Ints[colID,Row],True)+
...
procedure TfrmServices.asgKSGPartGetEditorType(Sender: TObject; ACol,
  ARow: Integer; var AEditor: TEditorType);
begin
   case ACol of
    colQty,colPrice,colDisc : AEditor := edPositiveNumeric;
...
procedure TfrmServices.asgKSGPartRightClickCell(Sender: TObject; ARow,
  ACol: Integer);
begin
//  if ARow = 0 then exit;
  //popReguler.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
...
procedure TfrmServices.asgRegServiceCanEditCell(Sender: TObject; ARow,
  ACol: Integer; var CanEdit: Boolean);
begin
...
procedure TfrmServices.asgRegServiceCellValidate(Sender: TObject; Col,
  Row: Integer; var Value: String; var Valid: Boolean);
var vMsg:string;
begin
  case Col of
...
procedure TfrmServices.asgRegServiceGetAlignment(Sender: TObject; ARow,
  ACol: Integer; var HAlign: TAlignment; var VAlign: TVAlignment);
begin
   if ARow = 0 then
    HAlign := taCenter
...
procedure TfrmServices.asgRegServiceGetEditorType(Sender: TObject; ACol,
  ARow: Integer; var AEditor: TEditorType);
...
procedure TfrmServices.DeleteRowGrid(AGrid: TAdvStringGrid; ARow,
  ACOL: integer);
begin
  if ARow = 0 then exit;
...
procedure TfrmServices.asgRegServiceRightClickCell(Sender: TObject; ARow,
  ACol: Integer);
begin
 // if ARow = 0 then exit;
 // popReguler.Popup(Mouse.CursorPos.X,Mouse.CursorPos.Y);
...
procedure TfrmServices.btnCustNewClick(Sender: TObject);
var tmp : integer;
begin
...
procedure TfrmServices.asgRegServiceKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
...
procedure TfrmServices.asgRegPartKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
...
procedure TfrmServices.getItem(AGrid: TAdvStringGrid; AItemId,
...
procedure TfrmServices.asgKSGServiceKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var vId : integer;vFilter,vSearch : string;
begin
  if BrowseMode and (not EditMode) then exit;
...
procedure TfrmServices.asgCompanyServiceKeyDown(Sender: TObject;
...
procedure TfrmServices.asgKSGPartKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var vId : integer; vSearch:string;
begin
  if BrowseMode and (not EditMode) then exit;
...
procedure TfrmServices.asgCompanyPartKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
var vId : integer; vSearch:string;
begin
   if BrowseMode and (not EditMode) then exit;
...
procedure TfrmServices.dtpTransKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmServices.chkPostKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_RETURN then
    btnSave.SetFocus;
...
procedure TfrmServices.cmbMekanikKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_RETURN then
    dtpBayar.SetFocus;
...
procedure TfrmServices.mmNotesKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_RETURN then
    pgcDetail.SetFocus;
...
procedure TfrmServices.btnPostingClick(Sender: TObject);
begin
...
procedure TfrmServices.asgKSGServiceCanEditCell(Sender: TObject; ARow,
  ACol: Integer; var CanEdit: Boolean);
begin
...
procedure TfrmServices.asgCompanyServiceCanEditCell(Sender: TObject; ARow,
  ACol: Integer; var CanEdit: Boolean);
begin
  CanEdit := (ACol in [colQty,colCode,colName,colPrice,colDisc,colDiscPct])
            and ((not BrowseMode) or EditMode) and (MasterShipService=SHIP_SERVICE_TYPE_COMP);
...
procedure TfrmServices.asgKSGServiceCellValidate(Sender: TObject; Col,
  Row: Integer; var Value: String; var Valid: Boolean);
var vMsg : string;
begin
case Col of
...
procedure TfrmServices.asgCompanyServiceCellValidate(Sender: TObject; Col,
  Row: Integer; var Value: String; var Valid: Boolean);
var vMsg : string;
begin
case Col of
...
procedure TfrmServices.txtBayarKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = vk_return then
    btnSave.SetFocus;
...
procedure TfrmServices.txtNoPolKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmServices.asgRegServiceSelectCell(Sender: TObject; ACol,
...
procedure TfrmServices.setGridSelection(AGrid:TAdvStringGrid);
begin
...
procedure TfrmServices.asgRegServiceEnter(Sender: TObject);
begin
  setGridSelection(sender as TAdvStringGrid);
end;

procedure TfrmServices.asgCompanyServiceExit(Sender: TObject);
begin
  setGridSelection(nil);
end;

procedure TfrmServices.CountSubtotal(ARow, ACol: integer;AGrid:TAdvStringGrid);
...
procedure TfrmServices.txtDPChange(Sender: TObject);
begin
  SetFooter;
end;

procedure TfrmServices.dtpBayarKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if Key = VK_RETURN then
    mmNotes.SetFocus;
...
procedure TfrmServices.asgKSGPartGetAlignment(Sender: TObject; ARow,
  ACol: Integer; var HAlign: TAlignment; var VAlign: TVAlignment);
begin
...
procedure TfrmServices.asgKSGServiceGetAlignment(Sender: TObject; ARow,
  ACol: Integer; var HAlign: TAlignment; var VAlign: TVAlignment);
begin
 if (ARow = 0) or (ACol = colKSg) then
    HAlign := taCenter
...
procedure TfrmServices.asgKSGServiceCheckBoxClick(Sender: TObject; ACol,
  ARow: Integer; State: Boolean);
begin
  SetAmount(asgKSGService,ARow);
end;

procedure TfrmServices.asgKSGPartCheckBoxClick(Sender: TObject; ACol,
  ARow: Integer; State: Boolean);
begin
SetAmount(asgKSGPart,ARow);
end;

procedure TfrmServices.loadMekanik;
...
begin
  ...
  for i:= 0 to RelationArr.Count-1 do begin
   frmMainMenu.Gauge.Progress := round((i+1)/(RelationArr.Count)*100);
    ...
    asgMekanik.SetCheckBoxState(0,i+1,(EditMode or BrowseMode) and ( TPersons(RelationArr[i]).isMechanicOnService(MasterSeq,RelationArr[i].PersonId)));
    asgMekanik.AddRow;
  end;
...
procedure TfrmServices.InitGridMekanik;
begin
...
procedure TfrmServices.asgMekanikCanEditCell(Sender: TObject; ARow,
  ACol: Integer; var CanEdit: Boolean);
begin
  CanEdit := (ACol = 0) and (ARow>0) and ((not BrowseMode) or EditMode);
end;

procedure TfrmServices.LoadCar(ANoPol: string);
...
begin
    ...
    if vCar.SelectInDB(i,ANoPol,true) then begin
...
procedure TfrmServices.cmbNoPolSelect(Sender: TObject);
begin
//alert(listNoPol.Names[cmbNoPol.itemIndex]);
  loadCar(listNoPol.Names[cmbNoPol.itemIndex]);
end;

procedure TfrmServices.btnAddKendaraanClick(Sender: TObject);
var id:integer;
begin
...
procedure TfrmServices.btnOrderClick(Sender: TObject);
begin
  MasterSeq := frmLOVRekap.ExecuteOrder;
  LoadOrder;
  mmNotes.SetFocus;
end;

procedure TfrmServices.LoadOrder;
var vShipment : TShipment;
   // vDetail   : TItemDetail_Arr;
    i,vRow:integer;
    AGrid,AGridService : TAdvStringGrid;
...
begin
   ...
   txtOrder.Text := vShipment.ShipNum;
   txtOrder.Tag := vShipment.ShipId;
...
procedure TfrmServices.asgRegPartKeyPress(Sender: TObject; var Key: Char);
begin
