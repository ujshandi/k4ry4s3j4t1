...
type
  TfrmSalesServiceRekap = class(TForm)
    MainPanel: TPanel;
    pnlFilter: TPanel;
    btnReset: TButton;
    Panel1: TPanel;
    ...
    procedure cmbTipeLaporanSelect(Sender: TObject);
...
var
  frmSalesServiceRekap: TfrmSalesServiceRekap;
...
const
      ...
      IDX_REKAP = 0;
      IDX_HISTORY_KENDARAAN = 1;
      IDX_GLOBAL = 2;
      IDX_PITULASI = 3;
      IDX_KHUSUS = 4;
      IDX_GLOBAL_PART = 5;
      IDX_GLOBAL_HISTORY = 6; //30.06.07
      IDX_MOTOR = 7;
...
uses MainMenu, Subroutines, un_ryu, UTransaction, Penerimaan, UConst,
  ...
  RekapKhususPrint, GlobalPartPrint;
...
procedure TfrmSalesServiceRekap.ArrangeColSize;
begin
  case MasterPurpose[1] of
    SHIP_TYPE_SALES,SHIP_TYPE_RETUR_SALES : begin
      ...
      case cmbTipeLaporan.ItemIndex of
         ...
         IDX_GLOBAL : begin
            asgRekap.ColWidths[colgPiutang2] := 0;
    ...
    SHIP_TYPE_CLAIM,SHIP_TYPE_RETUR_SERVICE,SHIP_TYPE_SERVICE : begin
      ...
      case cmbTipeLaporan.ItemIndex of
        ...
        IDX_HISTORY_KENDARAAN : begin
            asgRekap.ColWidths[colKId] := 0;
            asgRekap.ColWidths[colKPriceHpp] := 0;
            //asgRekap.AutoNumberCol(colKNo);
        end;
        IDX_KHUSUS : begin
            asgRekap.ColWidths[colKId] := 0;
...
function TfrmSalesServiceRekap.Execute(aPurpose:string;AMenuId:integer): boolean;
begin
 Result := True;
...
procedure TfrmSalesServiceRekap.InitForm;
begin


  btnEdit.Visible     := MasterPurpose[1] in[SHIP_TYPE_SERVICE,SHIP_TYPE_SALES];
  //rbUnitMotor.Visible := MasterPurpose[1] in[SHIP_TYPE_SERVICE];
  StaticText1.Font.Color := clBlack;
  pnlCust.Visible := (MasterPurpose[1] in[SHIP_TYPE_SALES,SHIP_TYPE_RETUR_SALES,SHIP_TYPE_RETUR_SERVICE,
            SHIP_TYPE_SERVICE,SHIP_TYPE_CLAIM]);
  InitFIlter;
  //rbRekap.Checked := True;
  setComboTipeLap;
  setComboTipeTrans;
  cmbTipeLaporan.ItemIndex := IDX_REKAP;
  cmbTipeTrans.ItemIndex := 0;
  cmbTipeLaporanSelect(Self);
  InitGrid;
  MainPanel.Update;
end;

procedure TfrmSalesServiceRekap.InitGrid;
begin
  asgRekap.ExpandAll;
  asgRekap.Clear;
...
//             Cells[colgDp,0]    := 'Uang Muka';
         ...
         IDX_HISTORY_KENDARAAN : begin
               ResetGrid(asgRekap,3,14,1,1);
              asgRekap.FloatingFooter.Visible := True;
              with asgRekap do begin
                 Cells[colKNo,0] := 'No.';
                 Cells[colKDate,0] := 'Tanggal.';
                 Cells[colKCust,0]    := 'Pelanggan';
                 Cells[colKPartCode,0]  := 'Kode Part/Service';
                 Cells[colKPartName,0]:= 'Nama Part/Service';
                 Cells[colKPriceSales,0] := 'Harga';
                 Cells[colKPriceHpp,0] := 'Harga HPP Satuan';
                 Cells[colKQty,0]  := 'Quantity';
                 Cells[colKDisc,0] := 'Discount';
                 Cells[colKTotal,0]:='Total';
                 Cells[colKId,0] := 'ID';
                 Cells[colKId+1,0] := 'Mekanik';
              end;
         end;
...
//             Cells[colgDp,0]    := 'Uang Muka';
       ...
       IDX_HISTORY_KENDARAAN : begin
          ...
          with asgRekap do begin
             ...
             Cells[colKDate,0] := 'Tanggal.';
             Cells[colKCust,0]    := 'Pelanggan';
             Cells[colKPartCode,0]  := 'Kode Part';
             Cells[colKPartName,0]:= 'Nama Part';
             Cells[colKPriceSales,0] := 'Harga';
             Cells[colKPriceHpp,0] := 'Harga HPP Satuan';
             Cells[colKQty,0]  := 'Quantity';
             Cells[colKDisc,0] := 'Discount';
...
procedure TfrmSalesServiceRekap.btnAddClick(Sender: TObject);
begin
  case MasterPurpose[1] of
...
procedure TfrmSalesServiceRekap.btnOkClick(Sender: TObject);
begin
  case MasterPurpose[1] of
    SHIP_TYPE_SERVICE : begin
      case cmbTipeLaporan.ItemIndex of
      IDX_REKAP   : ReloadService;
      IDX_HISTORY_KENDARAAN  : ReloadKhusus;
...
procedure TfrmSalesServiceRekap.btnDetailClick(Sender: TObject);
begin
  ...
  case MasterPurpose[1] of
    ...
    SHIP_TYPE_CLAIM : begin
      if asgRekap.Ints[colrId,asgRekap.Row] = 0 then exit;
      if frmClaim.Execute(asgRekap.Ints[colSeq,asgRekap.Row]) then
        btnOk.Click;
    end;
  end;
end;

procedure TfrmSalesServiceRekap.asgRekapGetAlignment(Sender: TObject; ARow,
    ...
    case MasterPurpose[1] of
      SHIP_TYPE_SERVICE,SHIP_TYPE_SALES : begin
        case cmbTipeLaporan.ItemIndex of
         ...
         IDX_KHUSUS,IDX_HISTORY_KENDARAAN : begin
          if ARow = 0  then
            HAlign := taCenter
...
procedure TfrmSalesServiceRekap.asgRekapDblClickCell(Sender: TObject; ARow,
  ACol: Integer);
begin
  if ARow = 0 then exit;
  btnDetail.Click;
...
procedure TfrmSalesServiceRekap.cmbOpr1Change(Sender: TObject);
begin
    dtpSampai1.Enabled := (cmbOpr1.ItemIndex = 3) and (cmbOpr1.ItemIndex <> 0);
end;

procedure TfrmSalesServiceRekap.InitFIlter;
begin
  rgPart.Enabled := MasterPurpose[1] in[SHIP_TYPE_SERVICE];
  chkPart.Enabled := MasterPurpose[1] in[SHIP_TYPE_SERVICE];
  chkPart.Checked := False;
  chkPartClick(Self);

  pnlPart.Enabled := False;
  txtKode.Clear;
  txtNama.Clear;
...
procedure TfrmSalesServiceRekap.btnFilterClick(Sender: TObject);
begin
  SetFilterSize(pnlFilter,btnFilter,150);
end;

procedure TfrmSalesServiceRekap.btnResetClick(Sender: TObject);
begin
    InitFIlter;
end;

procedure TfrmSalesServiceRekap.ReloadService;
var vShipment : TShipment_Arr;
   ...
   begin
      ...
      with vShipment.Filter do begin
        ...
        Shipment.Persons.PersonCode := UpperCase(txtNoPol.Text);
        Shipment.Persons.PersonName := UpperCase(txtNoPol.Text);
        Shipment.NoPol := UpperCase(txtNoPol.Text);
...
begin
   try
     ...
     SetFilter;
     vShipment.FindOnDb('',chkPending.Checked);
     InitGrid;
...
procedure TfrmSalesServiceRekap.ReloadReturService;
var vShipment : TShipment_Arr;
   i,vRow:integer;
...
procedure TfrmSalesServiceRekap.btnPrintClick(Sender: TObject);
var vFilter : TShipment;
   ...
   begin
      ...
      with vFilter do begin
        ...
        IsPost   := rgStatus.ItemIndex=1;

        ShipServiceType := getTransType;
  ...
  case MasterPurpose[1] of
    ...
    SHIP_TYPE_SERVICE : begin
      if cmbTipeLaporan.ItemIndex = IDX_REKAP then begin
        ...
        qrpServiceRekap.ExecutQuery(vFilter,chkPending.Checked,SHIP_TYPE_SERVICE);
...
//        Alert(MSG_UNDERCONSTRUCTION);
      ...
      if cmbTipeLaporan.ItemIndex = IDX_HISTORY_KENDARAAN then begin
        Application.CreateForm(TqrpRekapKhusus,qrpRekapKhusus);
        qrpRekapKhusus.Executes(asgRekap,MasterPurpose);
        qrpRekapKhusus.Free;
      end;
    ...
    SHIP_TYPE_SALES : begin
      if cmbTipeLaporan.ItemIndex = IDX_REKAP then begin
        ...
        qrpSalesRekap.ExecutQuery(vFilter,chkPending.Checked);
        qrpSalesRekap.Free;
      end;
      if cmbTipeLaporan.ItemIndex = IDX_GLOBAL then begin
      ...
      if (cmbTipeLaporan.ItemIndex in [IDX_KHUSUS,IDX_HISTORY_KENDARAAN]) then begin
        Application.CreateForm(TqrpRekapKhusus,qrpRekapKhusus);
        qrpRekapKhusus.Executes(asgRekap,MasterPurpose);
        qrpRekapKhusus.Free;
      end;
...
procedure TfrmSalesServiceRekap.btnDeleteClick(Sender: TObject);
begin
   if cmbTipeLaporan.ItemIndex <> IDX_REKAP then exit;
...
procedure TfrmSalesServiceRekap.ReloadSales;
var vShipment : TShipment_Arr;
   i,vRow:integer;
...
begin
   try
     ...
     for i:= 0 to vShipment.Count-1 do
       with asgRekap do begin
         ...
         Cells[colrExpr,vRow]:= NewFormatDate(vShipment[i].ExprDate);
         TPersons(vShipment[i].Persons).SelectInDB(vShipment[i].Persons.PersonId);
  ...
  //     Cells[coltNoPol,vRow]:= vShipment[i].Persons.PersonCode;
         ...
         Cells[colrTipe,vRow] :=TShipment.getDescTransType(vShipment[i].ShipType,vShipment[i].ShipServiceType);
         Cells[colrUser,vRow]:= vShipment[i].UserInsert;
         Cells[colrStatus,vRow]:= IfThen(vShipment[i].IsPost,'Posted','Pending');
...
procedure TfrmSalesServiceRekap.ReloadServiceSalesGlobal;
var vRow:integer;
  ...
  begin
    with asgRekap do  begin
      ...
      if chkPending.Checked then begin
        ...
        vPostCriteria := boNone;                                                                                  {diganti
                                   28.09.06     vPostCriteria}
      vBruto  := TShipment.getValueItemType('d.price*d.quantity',GlobalPeriode,MasterPurpose,AType,0,getTransType,boNone);
{diganti
                                   28.09.06     vPostCriteria}
      vDiskon := TShipment.getValueItemType('d.discount',GlobalPeriode,MasterPurpose,AType,0,getTransType,boTrue);
...
procedure TfrmSalesServiceRekap.btnEditClick(Sender: TObject);
begin
   if cmbTipeLaporan.ItemIndex <> IDX_REKAP then exit;
  if asgRekap.Row = 0 then exit;
  ...
  case MasterPurpose[1] of
    SHIP_TYPE_SERVICE : begin
      ...
      if frmServices.Execute(asgRekap.Ints[colsId,asgRekap.Row],True) then
        btnOk.Click;
    end;
   { SHIP_TYPE_RETUR_SERVICE : begin
      if asgRekap.Ints[colsId,asgRekap.Row] = 0 then exit;
    ...
    SHIP_TYPE_SALES : begin
      if asgRekap.Ints[colrId,asgRekap.Row] = 0 then exit;
      if TShipment.isPosted(asgRekap.Ints[colrId,asgRekap.Row]) then begin
        Alert('Data tidak bisa diedit');
        exit;
      end;
      if frmSales.Execute(asgRekap.Ints[colrId,asgRekap.Row],True) then
        btnOk.Click;
    end;
    {SHIP_TYPE_RETUR_SALES : begin
      if asgRekap.Ints[colrId,asgRekap.Row] = 0 then exit;
...
procedure TfrmSalesServiceRekap.ReloadUnitMotor;
var vMotor : TMotor_Arr;
    ...
    var j,arow : integer;
    begin
       vMotor.FindOnDb(True,getTransType);
...
//   SetPeriodeObject(GlobalPeriode,cmbOpr1.ItemIndex,dtpDari1,dtpSampai1);
   ...
   for i:= 1 to asgRekap.RowCount-2 do
     with asgRekap do begin
       Ints[coluQty,i]     := TShipment.getUnitMotor(Ints[coluId,i],getTransType);
       Floats[coluTotal,i] := TShipment.getTotalMotor(Ints[coluId,i],getTransType);
     end;
...
procedure TfrmSalesServiceRekap.chkPendingClick(Sender: TObject);
begin
  rgStatus.Enabled := chkPending.Checked;
  if not rgStatus.Enabled then
    rgStatus.ItemIndex :=  -1
  else
    rgStatus.ItemIndex := 0;
end;

procedure TfrmSalesServiceRekap.setComboTipeLap;
begin
  cmbTipeLaporan.Items.Clear;
  cmbTipeLaporan.Items.Add('Rekap Transaksi');
  cmbTipeLaporan.Items.Add('Histori Kendaraan');
 { cmbTipeLaporan.Items.Add('Global');
  ...
  case MasterPurpose[1] of
    SHIP_TYPE_SERVICE : begin
      cmbTipeLaporan.Items.Add('Per Unit Kendaraan');
    end;
  end;}
end;

procedure TfrmSalesServiceRekap.setComboTipeTrans;
begin
  cmbTipeTrans.Items.Clear;
  with cmbTipeTrans do begin
    Items.Add('Semua');
    case MasterPurpose[1] of
      SHIP_TYPE_SERVICE : begin
          Items.Add('Reguler');
...
function TfrmSalesServiceRekap.getTransType: string;
begin
  case MasterPurpose[1] of
    SHIP_TYPE_SALES : begin
      case cmbTipeTrans.ItemIndex of
        1 : Result := SHIP_SALES_TYPE_REG;
        2 : Result := SHIP_SALES_TYPE_CREDIT;
        else
          Result := '';
      end;
    end;
    SHIP_TYPE_SERVICE : begin
       case cmbTipeTrans.ItemIndex of
        1 : Result := SHIP_SERVICE_TYPE_REG;
        2 : Result := SHIP_SERVICE_TYPE_KSG;
        3 : Result := SHIP_SERVICE_TYPE_COMP;
        else
          Result := '';
      end;
    end;
...
procedure TfrmSalesServiceRekap.ReloadRekapitulasi;
...
procedure TfrmSalesServiceRekap.cmbTipeLaporanDropDown(Sender: TObject);
begin
   SetComboboxSize(cmbTipeLaporan,150);
end;

procedure TfrmSalesServiceRekap.cmbTipeLaporanCloseUp(Sender: TObject);
begin
  SetComboboxSize(cmbTipeLaporan,105);
end;

procedure TfrmSalesServiceRekap.cmbTipeLaporanSelect(Sender: TObject);
begin
  chkTglTrans.Enabled := True;
  if MasterPurpose = SHIP_TYPE_SERVICE then begin
    chkPending.Enabled     := cmbTipeLaporan.ItemIndex in [IDX_REKAP,IDX_MOTOR];
    cmbTipeTrans.Enabled := cmbTipeLaporan.ItemIndex in [IDX_REKAP,IDX_MOTOR];
    chkTglTrans.Enabled := cmbTipeLaporan.ItemIndex in [IDX_REKAP,IDX_KHUSUS,IDX_HISTORY_KENDARAAN];

    chkPart.Enabled := cmbTipeLaporan.ItemIndex in [IDX_GLOBAL,IDX_PITULASI,IDX_HISTORY_KENDARAAN];
    rgPart.Enabled  := cmbTipeLaporan.ItemIndex in [IDX_GLOBAL,IDX_PITULASI,IDX_HISTORY_KENDARAAN];
    pnlPart.Enabled := cmbTipeLaporan.ItemIndex in [IDX_REKAP,IDX_KHUSUS,IDX_GLOBAL_PART,IDX_GLOBAL_HISTORY,IDX_HISTORY_KENDARAAN];
    cmbTipeTrans.ItemIndex :=  0;
//    if cmbTipeLaporan.ItemIndex in [IDX_PITULASI,IDX_GLOBAL] then begin
  //    chkPending.Checked := False;
      chkPendingClick(Self);
   // end;
  end;
  if MasterPurpose = SHIP_TYPE_SALES then begin
    pnlPart.Enabled := cmbTipeLaporan.ItemIndex in [IDX_REKAP,IDX_KHUSUS,IDX_GLOBAL_PART,IDX_GLOBAL_HISTORY,IDX_HISTORY_KENDARAAN];
    chkPending.Enabled := cmbTipeLaporan.ItemIndex in [IDX_REKAP];
    chkTglTrans.Enabled := cmbTipeLaporan.ItemIndex in [IDX_REKAP];
  end;

  if (not chkTglTrans.Enabled) then begin
    chkTglTrans.Checked := False;
    chkTglTransClick(Self);
  end;

  if (not chkPending.Enabled) then begin
    chkPending.Checked := False;
    chkPendingClick(Self);
  end;
end;

procedure TfrmSalesServiceRekap.txtNoPolKeyDown(Sender: TObject;
...
procedure TfrmSalesServiceRekap.cmbOpr2Change(Sender: TObject);
begin
   dtpSampai2.Enabled := (cmbOpr2.ItemIndex = 3) and (cmbOpr2.ItemIndex <> 0);
end;

procedure TfrmSalesServiceRekap.chkTglTransClick(Sender: TObject);
begin
  pnlExpr.Enabled := chkTglTrans.Checked;
end;

procedure TfrmSalesServiceRekap.chkPartClick(Sender: TObject);
begin
  rgPart.Enabled := chkPart.Checked;
...
procedure TfrmSalesServiceRekap.txtKodeKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
var vId:integer;
begin
  if (Key = vk_return) or (Key = vk_tab) then begin
...
procedure TfrmSalesServiceRekap.txtNamaKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
var vId:integer;
begin
  if (Key = vk_return) or (Key = vk_tab) then begin
...
procedure TfrmSalesServiceRekap.ReloadKhusus;
var vShipment : TShipment_Arr;
...
begin
  ...
  InitGrid;
  SetFilter;
  vShipment.FindKhususWithService;
  getDistinctCust;
  vOldCust := '';
    ...
    for i:= 0 to vShipment.GBuffer.RecordCount - 1 do begin
      with vShipment.GBuffer do begin
        ...
        with asgRekap do begin
           ...
           Cells[colKCust,vRow2]    := BufferToString(Fields[10].Value)+' ('+BufferToString(Fields[9].Value) + ' km )';
           ...
           Cells[colKId+1,vRow2] := TShipmentMechanic_Arr.getMechanic(Fields[8].Value);
           AddRow;
        end;
        MoveNext;
      end;
...
procedure TfrmSalesServiceRekap.ReloadGlobalPart;
var vShipment : TShipment_Arr;
    i,vRow:integer;
  procedure SetFilter;
   begin
...
procedure TfrmSalesServiceRekap.ReloadKhusus2;
var vShipment : TShipment_Arr;
  i,j,vRow,vRow2,vDetailNo:integer;
